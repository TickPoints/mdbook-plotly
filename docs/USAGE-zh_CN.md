...
从 **v0.3.0** 开始，mdbook-plotly 提供两种二进制变体：
...
3. **Plot 生成器** —— 以问卷形式生成图表配置，无需手写 JSON。从八种图表类型中选择一种，填写逐条轨迹字段（名称、数据、类型、模式、颜色……），实时预览 JSON/TOML。一张图表可以包含**任意数量的轨迹**（`[` / `]` 切换轨迹，`a` 添加，`d` 删除），另有原始 JSON5 字段可直接编写 plot 块的 **`config`** 与 **`map`** 段。必填字段缺失或数字不合法时会内联标红。`c` 复制结果到剪贴板，`s` 保存为 `plot-<type>.json`/`.toml`，`r` 恢复内置示例。生成器完全离线可用：schema 内嵌在二进制中，无需任何网络拉取。
...
`input-type` 配置选项决定在 `plot`/`plotly` 代码块内部定义图表所使用的语法。当前支持的值：

- `json-input` – 基于 JSON5 的图表定义（推荐，也是当前唯一受支持的输入格式）
